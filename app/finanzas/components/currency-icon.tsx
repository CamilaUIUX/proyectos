...
/**
 * Ícono real de cada moneda, embebido (portado del repo de referencia).
 *
 ...
 *
 * Van inline y no como `<img src>`: no dependen de que un CDN siga vivo, no
 * agregan requests a cada pantalla, y no parpadean al cargar. Los cinco
 * juntos pesan ~4 KB.
 *
 * **No usan `<mask>` ni ningún `id` interno.** Las banderas venían recortadas
 * con una máscara SVG por id; al repetirse el mismo ícono en la página,
 * quedaban varios elementos con el mismo id y WebKit dejaba de resolver la
 * referencia — la bandera se dibujaba cuadrada en el iPhone. El círculo lo
 * recorta el contenedor por CSS: sin ids, no hay nada que colisionar.
 */
const ICONS: Record<Currency, { viewBox: string; body: ReactNode }> = {
  USD: {
    viewBox: '0 0 512 512',
    body: (
      <g>
...
export function CurrencyIcon({ currency, size = 28 }: { currency: Currency; size?: number }) {
  const icon = ICONS[currency]
  return (
    <span
      role="img"
      aria-label={LABELS[currency]}
      className="fz-currency-icon"
      style={{
        width: size,
        height: size,
        overflow: 'hidden',
